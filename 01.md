...
If the provider supports the `pov` but its results are not yet available (for example, scores are still being computed), the provider SHOULD respond with `202` and a `Retry-After` header. If the provider anticipates that for whatever reason it will be unable to serve results for the requested `pov` within a reasonable amount of time, the provider MUST respond with `422 Unprocessable Content` and SHOULD explain why in `X-Reason`. The provider MUST NOT substitute a different point of view (such as a global or default view) and present its results as personalized. Clients SHOULD surface this reason to their users and MAY fall back explicitly with an alternate request, e.g. one for a global view or one that substitutes the failed `pov` with an alternative `pov`, such as a client-specific default.
