...
| `source` | boolean | no | If `true`, the algorithm is personalized and requires a `source` pubkey in every request. If absent or `false`, the algorithm is global. |
...
```json
{
  "/reputation": [
      {
        "id": "pagerank",
        "description": "Global Pagerank computed on the Nostr social graph."
      },
      {
        "id": "personalized-pagerank",
        "description": "Personalized Pagerank, computed using the provided source pubkey.",
        "source": true
      }
    ],
  "/recommendations": [
...
Clients MUST specify a `source` pubkey in the request if the specified algorithm requires it in the capability document.  
Providers MUST respond with `422 Unprocessable Content` if a request does not include a `source` for an algorithm that requires one.

Clients MUST NOT send a `source` to an algorithm that doesn't require it. If a `source` is sent to a global algorithm, the provider MUST ignore it.

## Algorithm Variants

Providers SHOULD express algorithm variants as distinct named algorithms.
This avoids the need for algorithm-specific parameters that break interoperability.
